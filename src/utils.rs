...
mod tests {
    ...
    fn test_is_a_parent_of() {
        assert!(is_a_parent_of("/usr", "/usr/triet"));
        assert!(is_a_parent_of("/usr", "/usr/triet/i/am/descendant"));
        assert!(!is_a_parent_of("/usr", "/usr/."));
        assert!(!is_a_parent_of("/usr", "/usr/"));
        assert!(!is_a_parent_of("/usr", "/usr"));
        assert!(!is_a_parent_of("/usr/", "/usr"));
        assert!(!is_a_parent_of("/usr/triet", "/usr"));
        assert!(!is_a_parent_of("/usr/triet", "/usr/sibling"));
        assert!(!is_a_parent_of("/usr/folder", "/usr/folder_not_a_child"));
    }

    #[test]
    fn test_is_a_parent_of_root() {
        assert!(is_a_parent_of("/", "/usr/triet"));
