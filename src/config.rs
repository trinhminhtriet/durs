...
fn get_config_locations(base: &Path) -> Vec<PathBuf> {
    vec![
        base.join(".durs.toml"),
        base.join(".config").join("durs").join("config.toml"),
    ]
}
...
mod tests {
    ...
    fn test_get_depth() {
        ...
        let c = Config::default();
        let args = get_args(vec!["durs", "--depth", "5"]);
